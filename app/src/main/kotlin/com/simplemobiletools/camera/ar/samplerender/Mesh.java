 ...
 *
 * <p>To render the mesh, use SampleRender.draw().
 */
			...
			for (int i = 1; i < vertexBuffers.length; ++i) {
				...
				if (iterCount != vertexCount) {
					throw new IllegalStateException(
						String.format("Vertex buffers have mismatching numbers of vertices ([0] has %d but [%d] has %d)", vertexCount, i, iterCount));
				}
			}
