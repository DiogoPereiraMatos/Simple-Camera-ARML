...
public final class DisplayRotationHelper implements DisplayListener {
	...
	public float getCameraSensorRelativeViewportAspectRatio(String cameraId) {
		...
		aspectRatio = switch (cameraSensorToDisplayRotation) {
			case 90, 270 -> (float) viewportHeight / (float) viewportWidth;
			case 0, 180 -> (float) viewportWidth / (float) viewportHeight;
			default -> throw new RuntimeException("Unhandled rotation: " + cameraSensorToDisplayRotation);
		};
		return aspectRatio;
	}
	...
	private int toDegrees(int rotation) {
		return switch (rotation) {
			...
			default -> throw new RuntimeException("Unknown rotation " + rotation);
		};
	}
