 ...
 */
package com.simplemobiletools.camera.ar.helpers;
...
public final class SnackbarHelper {
	 ...
	 *
	 * <p>To use the root layout (ndroid.R.id.content), pass in {@code null}.
	 *
	 * @param snackbarView the view to pass to
	...
	private void show(
		...
		activity.runOnUiThread(
			new Runnable() {
				@Override
				public void run() {
					messageSnackbar = Snackbar.make(snackbarView == null ? activity.findViewById(android.R.id.content) : snackbarView, message, duration);
					...
					if (dismissBehavior != DismissBehavior.HIDE && duration == Snackbar.LENGTH_INDEFINITE) {
						...
						if (dismissBehavior == DismissBehavior.FINISH) {
							messageSnackbar.addCallback(new BaseTransientBottomBar.BaseCallback<>() {
								@Override
								public void onDismissed(Snackbar transientBottomBar, int event) {super.onDismissed(transientBottomBar, event);activity.finish();}
							});
						}
					}
