...
class HelloArView(val activity: ArActivity) : DefaultLifecycleObserver {
	...
	val arSettingsButton =
		root.findViewById<ImageButton>(R.id.ar_settings_button).apply {
	...
	fun showOcclusionDialogIfNeeded() {
		...
		AlertDialog.Builder(activity)
			...
			.setPositiveButton(R.string.button_text_enable_depth) { _, _ -> activity.depthSettings.setUseDepthForOcclusion(true) }
				.setNegativeButton(R.string.button_text_disable_depth) { _, _ -> activity.depthSettings.setUseDepthForOcclusion(false) }
					.show()
	}
	...
	private fun launchDepthSettingsMenuDialog() {
		...
		if (session.isDepthModeSupported(Config.DepthMode.AUTOMATIC)) {
			...
			AlertDialog.Builder(activity)
				.setTitle(R.string.options_title_with_depth)
				.setMultiChoiceItems(stringArray, checkboxes) { _, which, isChecked -> checkboxes[which] = isChecked }
					.setPositiveButton(R.string.done) { _, _ ->
